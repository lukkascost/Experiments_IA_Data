...
public class QuantityPerClassSplit extends SplitterModeDTO {
    ...
    private HashMap<String,Integer> quantityTrainPerClass;
    private HashMap<String,Integer> quantityTestPerClass;
    ...
    public SplitterDataset split(List<SampleEntity> samples) {
        ...
        for (int i = 0; i < classes.size(); i++) {
            ...
            int quatityTrain = this.quantityTrainPerClass.get(label);
            int quatityTest = this.quantityTestPerClass.get(label);
            ...
            result.getTrainSamples().addAll(entitiesFromClass.subList(0,quatityTrain).stream().map(x->x.getId()).collect(Collectors.toList()));
            result.getTestSamples().addAll(entitiesFromClass.subList(entitiesFromClass.size()-quatityTest,entitiesFromClass.size()).stream().map(x->x.getId()).collect(Collectors.toList()));
        }
        return result;
    }
}
